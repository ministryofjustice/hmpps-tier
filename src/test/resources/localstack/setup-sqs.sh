...
aws --endpoint-url=http://localhost:4575 sns create-topic --name hmpps-domain
...
aws --endpoint-url=http://localhost:4575 sns subscribe \
    --topic-arn arn:aws:sns:eu-west-2:000000000000:hmpps-domain \
    --protocol sqs \
    --notification-endpoint http://localhost:4576/queue/Digital-Prison-Services-dev-hmpps_tier_calculation_complete_queue \
    --attributes '{"FilterPolicy":"{\"eventType\":[\"TIER_CALCULATION_COMPLETE\"]}"}'

# example of how to publish a message from the cli

...
aws --endpoint-url=http://localstack:4566 sns create-topic --name offender_events

echo Starting sqs setup
aws --endpoint-url=http://localstack:4566 sqs create-queue --queue-name Digital-Prison-Services-dev-hmpps_tier_offender_events_queue
aws --endpoint-url=http://localstack:4566 sqs create-queue --queue-name Digital-Prison-Services-dev-hmpps_tier_offender_events_queue_d

aws --endpoint-url=http://localstack:4566 sqs create-queue --queue-name Digital-Prison-Services-dev-hmpps_tier_offender_events_queue
aws --endpoint-url=http://localstack:4566 sqs set-queue-attributes --queue-url "http://localstack:4576/queue/Digital-Prison-Services-dev-hmpps_tier_offender_events_queue" --attributes '{"RedrivePolicy":"{\"maxReceiveCount\":\"3\", \"deadLetterTargetArn\":\"arn:aws:sqs:eu-west-2:000000000000:Digital-Prison-Services-dev-hmpps_tier_offender_events_queue_d\"}"}'
aws --endpoint-url=http://localstack:4566 sns subscribe \
    --topic-arn arn:aws:sns:eu-west-2:000000000000:offender_events \
    --protocol sqs \
    --notification-endpoint http://localstack:4566/queue/Digital-Prison-Services-dev-hmpps_tier_offender_events_queue \

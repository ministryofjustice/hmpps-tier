...
class TierCalculationTest : IntegrationTestBase() {
  ...
  fun `no NSis returned Female Offender`() {
    ...
    tierToDeliusApi.getFullDetails(crn, TierDetails(gender = "Female", ogrsScore = null, rsrScore = null, convictions = listOf(Conviction())))
  ...
  fun `Does not write back when calculation result differs but tier is unchanged`() {
    ...
    tierToDeliusApi.getFullDetails(crn, TierDetails(ogrsScore = "0", convictions = listOf(Conviction(sentence = Sentence(sentenceCode = "SC")))))
    communityApi.getMappaRegistration(crn, "M2")
